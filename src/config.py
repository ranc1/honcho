...
ModelTransport = Literal["anthropic", "openai", "gemini", "acp"]
EmbeddingTransport = Literal["openai", "gemini", "openrouter"]
...
class LLMSettings(HonchoSettings):
    ...
    # ACP provider (fork) — routes LLM calls through an external ACP gateway
    # over HTTP instead of calling provider SDKs directly. See
    # .kiro/specs/acp-provider/.
...
class EmbeddingSettings(HonchoSettings):
    ...
    # Fork: defaults to 1024 to match qwen3-embedding:0.6b (the fork's
    # default embedding model). Operators using OpenAI text-embedding-3-*
    # should set EMBEDDING_VECTOR_DIMENSIONS=1536 explicitly.
