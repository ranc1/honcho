...
async def honcho_llm_call_inner(
    ...
    client_override: ProviderClient | str | None = None,
    ...
    if provider == "acp":
        ...
        if stream:
            ...
            text = (
                ...
                else str(acp_result.content)
            )

            async def _acp_single_chunk() -> AsyncIterator[HonchoLLMCallStreamChunk]:
                yield HonchoLLMCallStreamChunk(
                    content=text, is_done=True, finish_reasons=["stop"]
                )

            return _acp_single_chunk()

        return HonchoLLMCallResponse(
            content=acp_result.content,
            input_tokens=acp_result.input_tokens,
            output_tokens=acp_result.output_tokens,
            cache_creation_input_tokens=acp_result.cache_creation_input_tokens,
            cache_read_input_tokens=acp_result.cache_read_input_tokens,
            finish_reasons=acp_result.finish_reasons,
            tool_calls_made=acp_result.tool_calls_made,
            iterations=acp_result.iterations,
            thinking_content=acp_result.thinking_content,
            thinking_blocks=acp_result.thinking_blocks,
            reasoning_details=acp_result.reasoning_details,
        )
